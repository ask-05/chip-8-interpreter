...
CHIP8::CHIP8() : pc(PROGRAM_START), sp(0), delay_timer(0), sound_timer(0), rom_loaded(false) {
    ...
    last_timer_update = std::chrono::high_resolution_clock::now();
}
...
bool CHIP8::loadROM(const char* filename) {
    ...
    if (rom_bytes == 0) {
        cerr << "Error: ROM file is empty!" << endl;
        ROM.close();
        return false;
    }
    
    if (rom_bytes > MAX_ROM_SIZE) {
        cerr << "Error: ROM File Size (" << rom_bytes << " bytes) is too big! Max size: " << MAX_ROM_SIZE << " bytes" << endl;
        ROM.close();
        return false;
    }
    ...
    cout << "ROM loaded successfully: " << rom_bytes << " bytes" << endl;
...
void CHIP8::updateTimers() { 
    ...
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(current_time - last_timer_update).count();
...
void CHIP8::run() {
    ...
    while (running && rom_loaded) {
      // Handle SDL events
        ...
        auto render_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(current_render_time - last_render_time).count();
        ...
        auto current_time = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(current_time - last_cycle_time).count();
